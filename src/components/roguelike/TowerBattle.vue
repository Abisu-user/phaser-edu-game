  ...
  <div class="tower-battle h-full w-full flex flex-col bg-[#0B0B12] relative">
    ...
    <transition name="fade">
      <div v-if="showMenu" class="absolute inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md">
        <div class="w-80 bg-[#1A1A24] border border-[#2A2A40] rounded-2xl p-6 shadow-2xl flex flex-col gap-4 text-center">
          ...
          <button @click="showMenu = false" class="py-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold transition-all">
            繼續探索
          </button>
          
          <button @click="confirmExit('save')" class="py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold transition-all shadow-[0_0_15px_rgba(79,70,229,0.4)]">
            暫時中斷 (儲存並退出)
          </button>
          
          <button @click="confirmExit('abandon')" class="py-3 rounded-xl border border-rose-900/50 bg-rose-950/20 hover:bg-rose-600 text-rose-400 hover:text-white font-bold transition-all">
            終止連線 (放棄進度)
          </button>
        </div>
      </div>
    ...
    <header class="h-16 flex items-center justify-between px-6 border-b border-[#2A2A40] bg-[#131320]">
      ...
      <div class="flex gap-8 items-center">
        <div class="text-yellow-400 font-mono font-bold flex items-center gap-2">
          <span>💰</span> {{ coins }}
        </div>
        <div class="flex flex-col">
          <span class="text-xs text-indigo-400 font-bold uppercase tracking-wider mb-1">Level : {{ level }}</span>
        ...
        <div class="flex items-center gap-3">
          <span class="text-rose-500 font-black text-sm italic">HP : {{ hp }} / {{ maxHp }}</span>
          <div class="w-32 h-4 bg-black rounded-full overflow-hidden border border-slate-800 shadow-[inset_0_0_10px_rgba(0,0,0,0.8)]">
            <div class="h-full bg-gradient-to-r from-rose-700 to-rose-500 transition-all duration-300" :style="{ width: (hp / maxHp * 100) + '%' }"></div>
          </div>
          <span class="text-cyan-500 font-black text-sm italic ml-2">MP : {{ mp }} / {{ maxMp }}</span>
          <div class="w-32 h-4 bg-black rounded-full overflow-hidden border border-slate-800 shadow-[inset_0_0_10px_rgba(0,0,0,0.8)]">
            <div class="h-full bg-gradient-to-r from-cyan-700 to-cyan-500 transition-all duration-300" :style="{ width: (mp / maxMp * 100) + '%' }"></div>
          </div>
          <span class="text-fuchsia-500 font-black text-sm italic ml-2">AP : {{ ap }} / {{ maxAp }}</span>
          <div class="w-32 h-4 bg-black rounded-full overflow-hidden border border-slate-800 shadow-[inset_0_0_10px_rgba(0,0,0,0.8)]">
    ...
    <main class="flex-1 flex overflow-hidden p-4 gap-4">
      ...
      <aside class="w-[400px] bg-[#11111B]/90 backdrop-blur-xl border border-[#2A2A40] rounded-2xl flex flex-col overflow-hidden shadow-2xl">
        ...
        <div class="flex-1 p-4 overflow-hidden flex flex-col gap-4">
          ...
          <div class="h-1/2 flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto pr-2 space-y-2 custom-scrollbar">
              <details 
                v-for="(cat, index) in commandCategories" 
                :key="index"
                :open="cat.open"
                class="group bg-[#1A1A24]/80 rounded-xl border border-[#2A2A40] overflow-hidden transition-all duration-300"
              >
                <summary class="cursor-pointer p-3 text-sm font-bold text-slate-300 hover:text-white hover:bg-white/5 flex justify-between items-center select-none transition-colors">
                  <span>{{ cat.name }}</span>
                  <span class="group-open:rotate-180 transition-transform duration-300 text-slate-500 text-xs">▼</span>
                </summary>
                <div class="p-3 pt-1 grid grid-cols-2 gap-2 bg-[#0B0B12]/50 border-t border-[#2A2A40]/50">
                  <button 
                    v-for="cmd in cat.commands" 
                    :key="cmd.id"
                    @click="insertCode(cmd.id)"
                    class="text-xs py-2 px-2 rounded-lg border transition-all duration-300 text-left truncate flex items-center gap-1.5"
                    :class="getThemeClasses(cat.theme)"
                    :title="cmd.label"
                  >
                    {{ cmd.label }}
                  </button>
          ...
          <div class="h-1/2 flex flex-col bg-[#1E1E2E] rounded-xl border overflow-hidden relative shadow-inner transition-colors duration-300"
                :class="errorMessage ? 'border-rose-500 shadow-[0_0_15px_rgba(225,29,72,0.4)]' : 'border-slate-700/50'">
              ...
              <div class="flex justify-between items-center px-3 py-2 bg-[#181825] border-b border-slate-700/50 z-10 relative">
                ...
                <button @click="clearCode" class="text-[10px] text-rose-400 hover:text-white px-2 py-1 rounded bg-rose-950/30 border border-rose-900/50 hover:bg-rose-600 transition-colors">
                  清空程式碼
                </button>
              </div>
              ...
              <transition name="fade">
                <div v-if="errorMessage" 
                    class="absolute bottom-6 right-6 z-50 bg-rose-900/95 border border-rose-500/50 text-rose-200 px-4 py-3 rounded-lg shadow-xl flex items-center justify-between backdrop-blur-sm max-w-[80%] shadow-[0_4px_20px_rgba(225,29,72,0.3)]">
                  <span class="text-sm font-medium flex-1 mr-4">⚠️ {{ errorMessage }}</span>
                  <button @click="errorMessage = ''" class="text-rose-400 hover:text-white text-xl leading-none font-bold">
                    ×
                  </button>
                </div>
              </transition>
              ...
              <div class="flex-1 relative flex overflow-hidden">
               ...
               <textarea 
                  ...
                  @keydown="handleAutoClose"  @keyup="checkAutocomplete"
                  class="flex-1 w-full h-full p-4 bg-transparent text-indigo-300 font-mono text-sm resize-none focus:outline-none overflow-y-auto custom-scrollbar leading-relaxed"
                  placeholder="// 點擊上方模組，或在此輸入程式碼...&#10;// 例如: moveUp();"
                  spellcheck="false"
                ></textarea>
        ...
        <button 
          @click="executeCode"
          class="m-4 relative overflow-hidden py-3 rounded-xl font-black text-white transition-all duration-300 flex justify-center items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none active:scale-95 hover:-translate-y-0.5"
          :class="codeContent.trim().length > 0 && !errorMessage ? 'bg-gradient-to-r from-indigo-600 to-fuchsia-600 shadow-[0_0_20px_rgba(99,102,241,0.4)]' : 'bg-slate-800 border border-slate-700'"
          :disabled="codeContent.trim().length === 0 || isAiming || errorMessage !== ''"
        >
          ...
          <span class="relative z-10 tracking-widest">編譯並啟動機甲</span>
        </button>
      </aside>
    </main>
  </div>
...
const props = defineProps(['floor', 'hp', 'maxHp', 'mp', 'maxMp', 'ap', 'maxAp', 'coins', 'level', 'xp', 'totalExp']);
const emit = defineEmits(['stop', 'abandon', 'init-game', 'execute', 'update-stats']);

const showMenu = ref(false); 
// 統一使用 codeContent 作為編輯器綁定數值
...
const handleAutoClose = async (e) => {
  ...
  // 如果玩家按下的按鍵在我們的配對清單中 (例如按下了 '{')
  if (closeChar) {
    e.preventDefault(); // 阻止原本只輸入一個 '{' 的預設行為
    ...
    // Vue 更新 v-model 是非同步的，必須等畫面渲染完再移動游標
    await nextTick();
...
const confirmExit = (type) => {
  ...
  if (type === 'save') {
    ...
    if (confirm('警告：確定要終止連線嗎？目前的層數與局內金幣將會消失！')) {
      emit('abandon'); 
    }
  }
};
...
const insertCode = (commandId) => {
  const needTargetingSkills = [
    'attack', 'shoot', 'magic', 'bomb', 'laser', 
    'dash', 'hack_wall', 'pull', 'boomerang', 'spread_shot'
  ];
...
const executeCode = () => {
  ...
  const parsedCommands = rawCode
    ...
    .map(cmdStr => {
      ...
      if (match) {
        ...
        if (match[2].trim()) {
          try {
            // 解析傳入的參數，如果解析錯誤則給空陣列，防止崩潰
            args = new Function('return ' + match[2])() || {};
          } catch(e) {
            console.warn("參數解析失敗，忽略該參數:", match[2]);
          }
        }
  ...
  if (props.ap >= totalApCost) {
    ...
    // 這裡我們維持送出完整字串讓自定義直譯器或 evaluate 去執行
    // 如果底層 Phaser 的邏輯是接收 parsedCommands，請把 userCode 換成 parsedCommands
...
const commandCategories = computed(() => {
  ...
  return [
    { 
      ...
      theme: 'indigo', 
      open: true, 
      commands: COMMAND_DICT.filter(c => moveIds.includes(c.id)) 
    },
    { 
      name: '👁️ 條件與感知', 
      theme: 'emerald', 
      open: false, 
      commands: COMMAND_DICT.filter(c => c.type === 'sensor') 
    },
    { 
      name: '⚙️ 互動與機制', 
      theme: 'amber', 
      open: false, 
      commands: COMMAND_DICT.filter(c => interactIds.includes(c.id)) 
    },
    { 
      name: '⚔️ 攻擊與戰鬥', 
      theme: 'rose', 
      open: false, 
      commands: COMMAND_DICT.filter(c => combatIds.includes(c.id)) 
    },
    { 
      name: '🧠 邏輯控制', 
      theme: 'pink', 
      open: false, 
      commands: COMMAND_DICT.filter(c => c.type === 'logic') 
    }
...
const onPlayerHurt = (damage) => {
  const newHp = Math.max(0, props.hp - damage); 
  emit('update-stats', { hp: newHp });
};

const onPlayerHeal = (amount) => {
  const newHp = Math.min(props.maxHp, props.hp + amount); 
  emit('update-stats', { hp: newHp });
};

const onCoinCollected = (amount) => {
  const newCoins = props.coins + amount;
  emit('update-stats', { coins: newCoins });
};

const onLevelUp = (newLevel) => {
  emit('update-stats', { level: newLevel });
};

const onActionPointChange = (newAp) => {
  emit('update-stats', { ap: newAp });
};

const handleHealEvent = (e) => onPlayerHeal(e.detail.amount);
const handleHurtEvent = (e) => onPlayerHurt(e.detail.damage);
const handleCoinEvent = (e) => onCoinCollected(e.detail.amount);
const handleLevelUpEvent = (e) => onLevelUp(e.detail.level);
const handleActionPointChangeEvent = (e) => onActionPointChange(e.detail.ap);

onMounted(() => {
  emit('init-game');
  window.addEventListener('tower-player-hurt', handleHurtEvent);
  window.addEventListener('tower-coin-collected', handleCoinEvent);
  window.addEventListener('tower-level-up', handleLevelUpEvent);
  window.addEventListener('tower-action-point-change', handleActionPointChangeEvent);
  window.addEventListener('tower-player-heal', handleHealEvent);
  window.addEventListener('tower-target-selected', handleTargetSelected);
});

onUnmounted(() => {
  window.removeEventListener('tower-player-hurt', handleHurtEvent);
  window.removeEventListener('tower-coin-collected', handleCoinEvent);
  window.removeEventListener('tower-player-heal', handleHealEvent);
  window.removeEventListener('tower-action-point-change', handleActionPointChangeEvent);
