-- Keep every activity metric on the same active-student and Asia/Taipei day boundary.
...
begin
  ...
  return (
    ...
    range_progress as (
      ...
      where up.completed_at >= (v_start_date::timestamp at time zone 'Asia/Taipei')
        and up.completed_at < ((v_end_date + 1)::timestamp at time zone 'Asia/Taipei')
    ),
    ...
    completion as (
      select case when (select value from student_count) = 0 then 0
        else round(100.0 * count(*) / ((select value from student_count) * 25), 1)
      end as value
    ...
    daily_activity as (
      select
        ...
        count(range_progress.id)::integer as completions
      from generate_series(v_start_date, v_end_date, interval '1 day') as calendar(day)
      left join range_progress
        on range_progress.completed_at >= (calendar.day::date::timestamp at time zone 'Asia/Taipei')
       and range_progress.completed_at < ((calendar.day::date + 1)::timestamp at time zone 'Asia/Taipei')
    ...
    select jsonb_build_object(
      'summary', jsonb_build_object(
        'dau', (select active_users from daily_activity where day = v_end_date),
